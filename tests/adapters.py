...
def run_embedding(
    ...
    m = Embedding(vocab_size, d_model, device=weights.device, dtype=weights.dtype)
    m.load_state_dict({"weight": weights})
    return m(token_ids)
...
def run_swiglu(
    ...
    m = SwiGLU(d_model, d_ff, device=w1_weight.device, dtype=w1_weight.dtype)
...
def run_multihead_self_attention(
    ...
    m = MultiHeadSelfAttention(d_model, num_heads, device=q_proj_weight.device, dtype=q_proj_weight.dtype)
    m.load_state_dict(
        {
...
def run_transformer_block(
    ...
    w = weights["attn.q_proj.weight"]
    m = TransformerBlock(
        d_model, num_heads, d_ff, theta=theta, max_seq_len=max_seq_len, device=w.device, dtype=w.dtype
    )
    m.load_state_dict(weights)  # keys already match: ln1/attn.*_proj/ln2/ffn.w{1,2,3}
    return m(in_features)
...
def run_transformer_lm(
    ...
    m.load_state_dict(weights)  # keys already match the reference layout
    return m(in_indices)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    return train_bpe(input_path, vocab_size, special_tokens, num_processes=kwargs.get("num_processes"))
